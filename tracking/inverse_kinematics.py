...
class InverseKinematics:
    ...
    def solve(self, method: str = "lm"):
        ...
        if method != "lm" and method != "trf" and method != "only_lm":
            ...
            for ii in range(0, self.nb_frames):
                if inital_method != "lm":
                    x0 = np.random.uniform(initial_bounds[0], initial_bounds[1],
                                           self.nb_q) if ii == 0 else self.q[:, ii - 1]
                else:
    ...
    def animate(self):
        ...
        Animate the result of solve with bioviz.
        """
        b = bioviz.Viz(show_floor=False, loaded_model=self.biorbd_model, show_muscles=False)
